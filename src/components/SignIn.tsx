...
export default function SignIn({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Inform users about authorized domains if they are in the preview environment
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-md bg-surface-container-low p-8 rounded-3xl shadow-card">
        <h2 className="text-3xl font-headline font-bold text-on-surface mb-6 text-center">Sign In</h2>
        ...
        <p className="mt-6 text-center text-on-surface-variant text-sm">
          ...
          <button onClick={() => onNavigate('/signup')} className="text-primary font-bold">Sign Up</button>
        </p>
      </div>
    </div>
  );
